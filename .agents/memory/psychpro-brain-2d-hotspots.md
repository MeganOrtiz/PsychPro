...
**Rendering paradigm:** desktop "Sections" view = leader-line text-chip labels placed around all four edges of the brain (see `brain-lab-label-gutters.md`); mobile = numbered markers on the brain + a side `NumberedKey` (scrollable list, one row per hotspot). Hover/select is synced BOTH ways (`hoveredId` state on the page; chips/key rows and markers each emphasize on hover). Clicking either opens the existing `StructureDetail`. Section UI is gated to `viewMode === "sections"`; 3D/quiz modes fall back to `EmptyDetail` (desktop aside) and show no key on mobile.

**Color is UNIFORM, not per-category.** Per-structure / per-category color coding was REMOVED from the Brain Lab labels: every chip, badge, hotspot disc, glow and emphasized state uses the single locked cerulean `GLOW = #76E4F7`. There is NO category legend any more. `struct.color` still exists in the data (and is used elsewhere like StructureSearch/StructureDetail accents) but the brain labels must NOT reintroduce per-id/per-category color. Mint #5EEAD4 still FORBIDDEN app-wide.
