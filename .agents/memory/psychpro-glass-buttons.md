...
**Current standard = ROUNDED 14px OUTLINED-GLOW (cerulean).** Buttons app-wide use a glowing-OUTLINE vibe (inspired by an ElevenLabs ad): a lit 1.5px cerulean `#76E4F7` edge + soft multi-layer halo that *intensifies* idle → :hover → :active + 14px radius + surface-safe light cerulean tint fill (NOT a dark fill — buttons sit on BOTH the dark nebula AND white StudySurface cards). Carried by the `.btn-glass*` family in `src/index.css` (wired via `button.tsx` variant map) AND the legacy study-page `button.bg-primary`/`button.bg-secondary`/`button.border.border-input` recipes — keep both in lockstep, each with explicit `:active` rules. Keep `#76E4F7` only — NEVER mint `#5EEAD4`. **Why:** owner loved the landing `.landing-cta-primary` look and asked to roll it out everywhere; the `surface-hue` guardrail now enforces the cerulean family on buttons (see psychpro-sitewide-button-glow.md). **How to verify:** auth-gated buttons via an isolated mockup-sandbox copy mirroring exact index.css values across idle/hover/active (external Clerk blocks the test browser; a static screenshot can't trigger `:hover`/`:active`).

**Owner authors button hierarchy decisions.** Don't "correct" the landing hero CTA pairing toward a generic filled+outlined convention: the owner deliberately made BOTH hero CTAs filled (`JOIN NOW` + `BROWSE COURSES` = `landing-cta-primary`) to boost the secondary's prominence. Trust the latest committed landing state over any earlier "outlined pair" assumption.

**Archival — earlier superseded directions:** (1) a "neutral frost, no cyan" comp (IMG_2548), reverted; (2) a "square / `border-radius:0` true-0px corners" rule, superseded by the rounded-14px standard above. Owner flip-flopped over time, so always trust the most recent task spec / committed state over older notes.
