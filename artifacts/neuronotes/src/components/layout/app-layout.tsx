      ...
      >
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          <div className="px-3 pt-1 pb-1">
            <p className="text-xs font-semibold text-sidebar-foreground/40 uppercase tracking-wider">Study</p>
          </div>
          {studyNav.map((item) => {
          ...
          {!isScholar && (
            <>
              <div className="px-3 pt-3 pb-1">
                <p className="text-xs font-semibold text-sidebar-foreground/40 uppercase tracking-wider">Account</p>
              </div>
              {accountNav.map((item) => {
                const isActive = location === item.href || location.startsWith(item.href + "/");
                return (
