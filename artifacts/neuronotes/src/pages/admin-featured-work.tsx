              ...
              return (
                <div key={s.id} className="bg-card border border-border rounded-xl p-4" data-testid={`row-submission-${s.id}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {fileHref && (
                      <a href={fileHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground border border-border rounded-lg px-2.5 py-1">
                        <FileText className="w-3 h-3" /> PDF
                      </a>
                    )}
                    {s.externalLink && (
                      <a href={s.externalLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground border border-border rounded-lg px-2.5 py-1">
                        <ExternalLink className="w-3 h-3" /> Link
                      </a>
                    )}
                    <div className="flex-1" />
                    {s.status !== "approved" && (
                      <button onClick={() => applyAction(s.id, "approved")} disabled={busy} className="inline-flex items-center gap-1 text-xs text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800 rounded-lg px-2.5 py-1">
                        <Check className="w-3 h-3" /> Approve
                      </button>
                    )}
                    {s.status !== "revision_requested" && (
                      <button onClick={() => { setActionFor({ id: s.id, status: "revision_requested" }); setNote(s.adminNote ?? ""); }} className="inline-flex items-center gap-1 text-xs text-yellow-700 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-800 rounded-lg px-2.5 py-1">
                        <MessageCircle className="w-3 h-3" /> Request revision
                      </button>
                    )}
                    {s.status !== "rejected" && (
                      <button onClick={() => { setActionFor({ id: s.id, status: "rejected" }); setNote(s.adminNote ?? ""); }} className="inline-flex items-center gap-1 text-xs text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-lg px-2.5 py-1">
