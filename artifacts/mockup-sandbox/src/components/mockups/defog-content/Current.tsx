...
const MIST = "#d7eef5";
const MIST_SOFT = "rgba(196,232,242,0.62)";
...
const tileStyle: React.CSSProperties = {
  background:
    "radial-gradient(125% 80% at 50% 0%, rgba(118,228,247,0.08) 0%, rgba(118,228,247,0.00) 58%), linear-gradient(145deg, hsl(var(--surf-hue) 88% 23% / 0.93), hsl(var(--surf-hue) 88% 16% / 0.97))",
  borderColor: "rgba(196,232,242,0.26)",
  backdropFilter: "blur(5px) saturate(140%)",
  WebkitBackdropFilter: "blur(5px) saturate(140%)",
  boxShadow:
    "inset 0 1px 0 rgba(255,255,255,0.12), inset 0 0 34px -24px rgba(118,228,247,0.18), 0 0 18px -10px rgba(118,228,247,0.12), 0 16px 40px -30px rgba(0,0,0,0.74)",
};
    ...
    >
      ...
      <div className="space-y-5">
        <section className="relative rounded-2xl border p-5" style={sectionStyle}>
          ...
          <div className="mb-4 h-2 overflow-hidden rounded-full" style={{ background: "hsl(var(--surf-hue) 78% 48% / 0.14)" }}>
            <div className="h-full" style={{ width: "8%", background: `linear-gradient(90deg, ${TEAL_DEEP}, #b8f1fb)` }} />
          </div>
          <button className="inline-flex items-center gap-1 rounded-md px-5 py-2 text-sm font-semibold" style={{ background: "rgba(118,228,247,0.16)", color: MIST, border: "1px solid rgba(118,228,247,0.34)" }}>
            ...
            {[
              ...
              return (
                <div key={t.name} className="flex items-center gap-3 rounded-md border p-3" style={tileStyle}>
                  <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border" style={{ background: "rgba(118,228,247,0.10)", borderColor: "rgba(118,228,247,0.28)" }}>
          ...
          <div className="space-y-2">
            {["Scholar 0749", "Scholar 1785", "Scholar 5789", "Scholar 7135"].map((s, i) => (
              <div key={s} className="flex items-center gap-3 text-sm">
                <span className="flex h-5 w-5 items-center justify-center rounded-full text-[11px] font-bold" style={{ background: "rgba(118,228,247,0.16)", color: TEAL_DEEP }}>{i + 1}</span>
