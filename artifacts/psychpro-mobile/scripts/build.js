...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  // Clerk publishable key: prefer an explicit key, then the production web key,
  // then the dev key. The bundle is useless without one — fail loudly.
  const clerkKey =
    process.env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY ||
    process.env.VITE_CLERK_PUBLISHABLE_KEY ||
    process.env.VITE_CLERK_PUBLISHABLE_KEY_DEV;
  if (!clerkKey) {
    exitWithError(
      'ERROR: No Clerk publishable key found. Set EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY (or VITE_CLERK_PUBLISHABLE_KEY).',
    );
  }
